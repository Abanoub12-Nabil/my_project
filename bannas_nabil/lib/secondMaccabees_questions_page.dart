...
class SecondMaccabeesQuestionsPage extends StatefulWidget {
  const SecondMaccabeesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondMaccabeesQuestionsPageState createState() =>
      _SecondMaccabeesQuestionsPageState();
}

class _SecondMaccabeesQuestionsPageState
    extends State<SecondMaccabeesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondMaccabeesQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر المكابيين الثاني'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondMaccabeesQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .secondMaccabeesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
