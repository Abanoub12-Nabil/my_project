...
class FirstMaccabeesQuestionsPage extends StatefulWidget {
  const FirstMaccabeesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _FirstMaccabeesQuestionsPageState createState() =>
      _FirstMaccabeesQuestionsPageState();
}

class _FirstMaccabeesQuestionsPageState
    extends State<FirstMaccabeesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.firstMaccabeesQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر المكابيين الأول'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.firstMaccabeesQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .firstMaccabeesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
