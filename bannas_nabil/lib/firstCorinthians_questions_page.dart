...
class FirstCorinthiansQuestionsPage extends StatefulWidget {
  const FirstCorinthiansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _FirstCorinthiansQuestionsPageState createState() =>
      _FirstCorinthiansQuestionsPageState();
}

class _FirstCorinthiansQuestionsPageState
    extends State<FirstCorinthiansQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.firstCorinthiansQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة الأولي إلي كورنثوس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.firstCorinthiansQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .firstCorinthiansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
