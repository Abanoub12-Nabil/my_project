...
class SecondKingsQuestionsPage extends StatefulWidget {
  const SecondKingsQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondKingsQuestionsPageState createState() =>
      _SecondKingsQuestionsPageState();
}

class _SecondKingsQuestionsPageState extends State<SecondKingsQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondKingsQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر الملوك الثاني'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondKingsQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.secondKingsQuestions[chapter]!,
                      ),
                    );
                  },
                ),
