...
class EzraQuestionsPage extends StatefulWidget {
  const EzraQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _EzraQuestionsPageState createState() => _EzraQuestionsPageState();
}

class _EzraQuestionsPageState extends State<EzraQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.ezraQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر عزرا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.ezraQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.ezraQuestions[chapter]!,
                      ),
                    );
                  },
                ),
