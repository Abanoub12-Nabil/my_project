...
class ThirdJohnQuestionsPage extends StatefulWidget {
  const ThirdJohnQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ThirdJohnQuestionsPageState createState() => _ThirdJohnQuestionsPageState();
}

class _ThirdJohnQuestionsPageState extends State<ThirdJohnQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.thirdJohnQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رسالة يوحنا الثالثة'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.thirdJohnQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.thirdJohnQuestions[chapter]!,
                      ),
                    );
                  },
                ),
