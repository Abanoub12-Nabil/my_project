...
class RomansQuestionsPage extends StatefulWidget {
  const RomansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _RomansQuestionsPageState createState() => _RomansQuestionsPageState();
}

class _RomansQuestionsPageState extends State<RomansQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.romansQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلي رومية'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.romansQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.romansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
