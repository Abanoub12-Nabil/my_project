...
class WisdomQuestionsPage extends StatefulWidget {
  const WisdomQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _WisdomQuestionsPageState createState() => _WisdomQuestionsPageState();
}

class _WisdomQuestionsPageState extends State<WisdomQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.wisdomQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر الحكمة'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.wisdomQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.wisdomQuestions[chapter]!,
                      ),
                    );
                  },
                ),
