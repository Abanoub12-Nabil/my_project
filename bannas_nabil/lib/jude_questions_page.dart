...
class JudeQuestionsPage extends StatefulWidget {
  const JudeQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JudeQuestionsPageState createState() => _JudeQuestionsPageState();
}

class _JudeQuestionsPageState extends State<JudeQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.judeQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رسالة يهوذا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.judeQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.judeQuestions[chapter]!,
                      ),
                    );
                  },
                ),
