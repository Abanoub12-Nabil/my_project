...
class TitusQuestionsPage extends StatefulWidget {
  const TitusQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _TitusQuestionsPageState createState() => _TitusQuestionsPageState();
}

class _TitusQuestionsPageState extends State<TitusQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.titusQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلى تيطس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.titusQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.titusQuestions[chapter]!,
                      ),
                    );
                  },
                ),
