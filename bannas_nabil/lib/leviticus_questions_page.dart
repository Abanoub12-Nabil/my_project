...
class LeviticusQuestionsPage extends StatefulWidget {
  const LeviticusQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _LeviticusQuestionsPageState createState() => _LeviticusQuestionsPageState();
}

class _LeviticusQuestionsPageState extends State<LeviticusQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.leviticusQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر اللاويين'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.leviticusQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.leviticusQuestions[chapter]!,
                      ),
                    );
                  },
                ),
