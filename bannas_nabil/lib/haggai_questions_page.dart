...
class HaggaiQuestionsPage extends StatefulWidget {
  const HaggaiQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _HaggaiQuestionsPageState createState() => _HaggaiQuestionsPageState();
}

class _HaggaiQuestionsPageState extends State<HaggaiQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.haggaiQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر حجي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.haggaiQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.haggaiQuestions[chapter]!,
                      ),
                    );
                  },
                ),
