...
class SirachQuestionsPage extends StatefulWidget {
  const SirachQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SirachQuestionsPageState createState() => _SirachQuestionsPageState();
}

class _SirachQuestionsPageState extends State<SirachQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.sirachQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر يشوع ابن سيراخ'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.sirachQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.sirachQuestions[chapter]!,
                      ),
                    );
                  },
                ),
