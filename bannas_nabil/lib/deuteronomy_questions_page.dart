...
class DeuteronomyQuestionsPage extends StatefulWidget {
  const DeuteronomyQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _DeuteronomyQuestionsPageState createState() =>
      _DeuteronomyQuestionsPageState();
}

class _DeuteronomyQuestionsPageState extends State<DeuteronomyQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.deuteronomyQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر التثنية'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.deuteronomyQuestions[chapter]?.length ?? 0;
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.deuteronomyQuestions[chapter]!,
                      ),
                    );
                  },
                ),
