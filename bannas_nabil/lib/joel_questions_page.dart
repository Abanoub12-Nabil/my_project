...
class JoelQuestionsPage extends StatefulWidget {
  const JoelQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JoelQuestionsPageState createState() => _JoelQuestionsPageState();
}

class _JoelQuestionsPageState extends State<JoelQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.joelQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر يوئيل'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.joelQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.joelQuestions[chapter]!,
                      ),
                    );
                  },
                ),
