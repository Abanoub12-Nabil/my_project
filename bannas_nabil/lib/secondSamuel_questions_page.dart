...
class SecondSamuelQuestionsPage extends StatefulWidget {
  const SecondSamuelQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondSamuelQuestionsPageState createState() =>
      _SecondSamuelQuestionsPageState();
}

class _SecondSamuelQuestionsPageState extends State<SecondSamuelQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondSamuelQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر صموئيل الثاني'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondSamuelQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.secondSamuelQuestions[chapter]!,
                      ),
                    );
                  },
                ),
