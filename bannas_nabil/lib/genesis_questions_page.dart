...
class GenesisQuestionsPage extends StatefulWidget {
  const GenesisQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _GenesisQuestionsPageState createState() => _GenesisQuestionsPageState();
}

class _GenesisQuestionsPageState extends State<GenesisQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.genesisQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر التكوين'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.genesisQuestions[chapter]?.length ?? 0;
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.genesisQuestions[chapter]!,
                      ),
                    );
                  },
                ),
