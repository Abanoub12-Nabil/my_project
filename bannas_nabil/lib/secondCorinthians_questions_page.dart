...
class SecondCorinthiansQuestionsPage extends StatefulWidget {
  const SecondCorinthiansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondCorinthiansQuestionsPageState createState() =>
      _SecondCorinthiansQuestionsPageState();
}

class _SecondCorinthiansQuestionsPageState
    extends State<SecondCorinthiansQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondCorinthiansQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة الثانية إلي كورنثوس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondCorinthiansQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .secondCorinthiansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
