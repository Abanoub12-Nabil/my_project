...
class JonahQuestionsPage extends StatefulWidget {
  const JonahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JonahQuestionsPageState createState() => _JonahQuestionsPageState();
}

class _JonahQuestionsPageState extends State<JonahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.jonahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر يونان'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.jonahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.jonahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
