...
class EphesiansQuestionsPage extends StatefulWidget {
  const EphesiansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _EphesiansQuestionsPageState createState() => _EphesiansQuestionsPageState();
}

class _EphesiansQuestionsPageState extends State<EphesiansQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.ephesiansQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلي أفسس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.ephesiansQuestions[chapter]?.length ??
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.ephesiansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
