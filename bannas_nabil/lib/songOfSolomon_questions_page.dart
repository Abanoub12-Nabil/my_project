...
class SongOfSolomonQuestionsPage extends StatefulWidget {
  const SongOfSolomonQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SongOfSolomonQuestionsPageState createState() =>
      _SongOfSolomonQuestionsPageState();
}

class _SongOfSolomonQuestionsPageState
    extends State<SongOfSolomonQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.songOfSolomonQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر نشيد الأناشيد'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.songOfSolomonQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .songOfSolomonQuestions[chapter]!,
                      ),
                    );
                  },
                ),
