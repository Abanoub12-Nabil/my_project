...
class RuthQuestionsPage extends StatefulWidget {
  const RuthQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _RuthQuestionsPageState createState() => _RuthQuestionsPageState();
}

class _RuthQuestionsPageState extends State<RuthQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.ruthQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر راعوث'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.ruthQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.ruthQuestions[chapter]!,
                      ),
                    );
                  },
                ),
