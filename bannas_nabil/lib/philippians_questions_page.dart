...
class PhilippiansQuestionsPage extends StatefulWidget {
  const PhilippiansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _PhilippiansQuestionsPageState createState() =>
      _PhilippiansQuestionsPageState();
}

class _PhilippiansQuestionsPageState extends State<PhilippiansQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.philippiansQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلي فيلبي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.philippiansQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.philippiansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
