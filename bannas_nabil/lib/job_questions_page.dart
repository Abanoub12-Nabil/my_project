...
class JobQuestionsPage extends StatefulWidget {
  const JobQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JobQuestionsPageState createState() => _JobQuestionsPageState();
}

class _JobQuestionsPageState extends State<JobQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.jobQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر أيوب'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.jobQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.jobQuestions[chapter]!,
                      ),
                    );
                  },
                ),
