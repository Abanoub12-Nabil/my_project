...
class FirstChroniclesQuestionsPage extends StatefulWidget {
  const FirstChroniclesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _FirstChroniclesQuestionsPageState createState() =>
      _FirstChroniclesQuestionsPageState();
}

class _FirstChroniclesQuestionsPageState
    extends State<FirstChroniclesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.firstChroniclesQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر أخبار الأيام الأول'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.firstChroniclesQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .firstChroniclesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
