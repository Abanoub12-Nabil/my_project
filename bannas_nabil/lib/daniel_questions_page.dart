...
class DanielQuestionsPage extends StatefulWidget {
  const DanielQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _DanielQuestionsPageState createState() => _DanielQuestionsPageState();
}

class _DanielQuestionsPageState extends State<DanielQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.danielQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر دانيال'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.danielQuestions[chapter]?.length ?? 0;
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.danielQuestions[chapter]!,
                      ),
                    );
                  },
                ),
