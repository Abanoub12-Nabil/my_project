...
class BaruchQuestionsPage extends StatefulWidget {
  const BaruchQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _BaruchQuestionsPageState createState() => _BaruchQuestionsPageState();
}

class _BaruchQuestionsPageState extends State<BaruchQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.baruchQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر باروخ'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.baruchQuestions[chapter]?.length ?? 0;
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.baruchQuestions[chapter]!,
                      ),
                    );
                  },
                ),
