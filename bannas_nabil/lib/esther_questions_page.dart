...
class EstherQuestionsPage extends StatefulWidget {
  const EstherQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _EstherQuestionsPageState createState() => _EstherQuestionsPageState();
}

class _EstherQuestionsPageState extends State<EstherQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.estherQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر أستير'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.estherQuestions[chapter]?.length ??
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.estherQuestions[chapter]!,
                      ),
                    );
                  },
                ),
