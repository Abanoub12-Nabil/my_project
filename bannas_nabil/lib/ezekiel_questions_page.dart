...
class EzekielQuestionsPage extends StatefulWidget {
  const EzekielQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _EzekielQuestionsPageState createState() => _EzekielQuestionsPageState();
}

class _EzekielQuestionsPageState extends State<EzekielQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.ezekielQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر حزقيال'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.ezekielQuestions[chapter]?.length ??
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.ezekielQuestions[chapter]!,
                      ),
                    );
                  },
                ),
