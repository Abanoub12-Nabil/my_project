...
class ProverbsQuestionsPage extends StatefulWidget {
  const ProverbsQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ProverbsQuestionsPageState createState() => _ProverbsQuestionsPageState();
}

class _ProverbsQuestionsPageState extends State<ProverbsQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.proverbsQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر أمثال سليمان'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.proverbsQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.proverbsQuestions[chapter]!,
                      ),
                    );
                  },
                ),
