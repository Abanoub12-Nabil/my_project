...
class LamentationsQuestionsPage extends StatefulWidget {
  const LamentationsQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _LamentationsQuestionsPageState createState() =>
      _LamentationsQuestionsPageState();
}

class _LamentationsQuestionsPageState extends State<LamentationsQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.lamentationsQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر مراثي إرميا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.lamentationsQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.lamentationsQuestions[chapter]!,
                      ),
                    );
                  },
                ),
