...
class MalachiQuestionsPage extends StatefulWidget {
  const MalachiQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _MalachiQuestionsPageState createState() => _MalachiQuestionsPageState();
}

class _MalachiQuestionsPageState extends State<MalachiQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.malachiQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر ملاخي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.malachiQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.malachiQuestions[chapter]!,
                      ),
                    );
                  },
                ),
