...
class MarkQuestionsPage extends StatefulWidget {
  const MarkQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _MarkQuestionsPageState createState() => _MarkQuestionsPageState();
}

class _MarkQuestionsPageState extends State<MarkQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.markQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة إنجيل مرقس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.markQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.markQuestions[chapter]!,
                      ),
                    );
                  },
                ),
