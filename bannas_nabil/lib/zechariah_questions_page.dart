...
class ZechariahQuestionsPage extends StatefulWidget {
  const ZechariahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ZechariahQuestionsPageState createState() => _ZechariahQuestionsPageState();
}

class _ZechariahQuestionsPageState extends State<ZechariahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.zechariahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر زكريا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.zechariahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.zechariahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
