...
class ActsQuestionsPage extends StatefulWidget {
  const ActsQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ActsQuestionsPageState createState() => _ActsQuestionsPageState();
}

class _ActsQuestionsPageState extends State<ActsQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.actsQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر أعمال الرسل'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.actsQuestions[chapter]?.length ?? 0;
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.actsQuestions[chapter]!,
                      ),
                    );
                  },
                ),
