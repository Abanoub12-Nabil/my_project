...
class SecondThessaloniansQuestionsPage extends StatefulWidget {
  const SecondThessaloniansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondThessaloniansQuestionsPageState createState() =>
      _SecondThessaloniansQuestionsPageState();
}

class _SecondThessaloniansQuestionsPageState
    extends State<SecondThessaloniansQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondThessaloniansQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة الثانية إلي تسالونيكي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount = QuestionsData
              .secondThessaloniansQuestions[chapter]?.length ??
              0; // عدد الأسئلة
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .secondThessaloniansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
