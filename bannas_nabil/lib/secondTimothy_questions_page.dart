...
class SecondTimothyQuestionsPage extends StatefulWidget {
  const SecondTimothyQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondTimothyQuestionsPageState createState() =>
      _SecondTimothyQuestionsPageState();
}

class _SecondTimothyQuestionsPageState
    extends State<SecondTimothyQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondTimothyQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة الثانية إلي تيموثاوس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondTimothyQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .secondTimothyQuestions[chapter]!,
                      ),
                    );
                  },
                ),
