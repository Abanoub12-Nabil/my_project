...
class GalatiansQuestionsPage extends StatefulWidget {
  const GalatiansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _GalatiansQuestionsPageState createState() => _GalatiansQuestionsPageState();
}

class _GalatiansQuestionsPageState extends State<GalatiansQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.galatiansQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلي غلاطية'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.galatiansQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.galatiansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
