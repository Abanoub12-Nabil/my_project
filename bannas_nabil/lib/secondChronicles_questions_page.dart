...
class SecondChroniclesQuestionsPage extends StatefulWidget {
  const SecondChroniclesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondChroniclesQuestionsPageState createState() =>
      _SecondChroniclesQuestionsPageState();
}

class _SecondChroniclesQuestionsPageState
    extends State<SecondChroniclesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondChroniclesQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر أخبار الأيام الثاني'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondChroniclesQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .secondChroniclesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
