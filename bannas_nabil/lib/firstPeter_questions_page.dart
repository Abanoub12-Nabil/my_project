...
class FirstPeterQuestionsPage extends StatefulWidget {
  const FirstPeterQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _FirstPeterQuestionsPageState createState() =>
      _FirstPeterQuestionsPageState();
}

class _FirstPeterQuestionsPageState extends State<FirstPeterQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.firstPeterQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رسالة بطرس الأولي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.firstPeterQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.firstPeterQuestions[chapter]!,
                      ),
                    );
                  },
                ),
