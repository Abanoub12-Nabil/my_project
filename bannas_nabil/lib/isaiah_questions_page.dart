...
class IsaiahQuestionsPage extends StatefulWidget {
  const IsaiahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _IsaiahQuestionsPageState createState() => _IsaiahQuestionsPageState();
}

class _IsaiahQuestionsPageState extends State<IsaiahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.isaiahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر إشعياء'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.isaiahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.isaiahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
