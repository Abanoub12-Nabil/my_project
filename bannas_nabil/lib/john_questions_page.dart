...
class JohnQuestionsPage extends StatefulWidget {
  const JohnQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JohnQuestionsPageState createState() => _JohnQuestionsPageState();
}

class _JohnQuestionsPageState extends State<JohnQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.johnQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة إنجيل يوحنا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.johnQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.johnQuestions[chapter]!,
                      ),
                    );
                  },
                ),
