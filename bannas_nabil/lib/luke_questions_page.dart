...
class LukeQuestionsPage extends StatefulWidget {
  const LukeQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _LukeQuestionsPageState createState() => _LukeQuestionsPageState();
}

class _LukeQuestionsPageState extends State<LukeQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.lukeQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة إنجيل لوقا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.lukeQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.lukeQuestions[chapter]!,
                      ),
                    );
                  },
                ),
