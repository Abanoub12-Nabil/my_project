...
class AmosQuestionsPage extends StatefulWidget {
  const AmosQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _AmosQuestionsPageState createState() => _AmosQuestionsPageState();
}

class _AmosQuestionsPageState extends State<AmosQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.amosQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر عاموس'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.amosQuestions[chapter]?.length ?? 0;
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.amosQuestions[chapter]!,
                      ),
                    );
                  },
                ),
