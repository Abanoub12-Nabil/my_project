...
class HabakkukQuestionsPage extends StatefulWidget {
  const HabakkukQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _HabakkukQuestionsPageState createState() => _HabakkukQuestionsPageState();
}

class _HabakkukQuestionsPageState extends State<HabakkukQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.habakkukQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر حبقوق'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.habakkukQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.habakkukQuestions[chapter]!,
                      ),
                    );
                  },
                ),
