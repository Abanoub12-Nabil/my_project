...
class JoshuaQuestionsPage extends StatefulWidget {
  const JoshuaQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JoshuaQuestionsPageState createState() => _JoshuaQuestionsPageState();
}

class _JoshuaQuestionsPageState extends State<JoshuaQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.joshuaQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر يشوع'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.joshuaQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.joshuaQuestions[chapter]!,
                      ),
                    );
                  },
                ),
