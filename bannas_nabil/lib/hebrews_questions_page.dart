...
class HebrewsQuestionsPage extends StatefulWidget {
  const HebrewsQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _HebrewsQuestionsPageState createState() => _HebrewsQuestionsPageState();
}

class _HebrewsQuestionsPageState extends State<HebrewsQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.hebrewsQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلي العبرانيين'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.hebrewsQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.hebrewsQuestions[chapter]!,
                      ),
                    );
                  },
                ),
