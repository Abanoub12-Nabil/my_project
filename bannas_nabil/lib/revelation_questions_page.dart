...
class RevelationQuestionsPage extends StatefulWidget {
  const RevelationQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _RevelationQuestionsPageState createState() =>
      _RevelationQuestionsPageState();
}

class _RevelationQuestionsPageState extends State<RevelationQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.revelationQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رؤيا يوحنا اللاهوتي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.revelationQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.revelationQuestions[chapter]!,
                      ),
                    );
                  },
                ),
