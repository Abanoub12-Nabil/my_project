...
class ZephaniahQuestionsPage extends StatefulWidget {
  const ZephaniahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ZephaniahQuestionsPageState createState() => _ZephaniahQuestionsPageState();
}

class _ZephaniahQuestionsPageState extends State<ZephaniahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.zephaniahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر صفنيا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.zephaniahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.zephaniahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
