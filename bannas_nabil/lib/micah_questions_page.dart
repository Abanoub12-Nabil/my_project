...
class MicahQuestionsPage extends StatefulWidget {
  const MicahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _MicahQuestionsPageState createState() => _MicahQuestionsPageState();
}

class _MicahQuestionsPageState extends State<MicahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.micahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر ميخا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.micahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.micahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
