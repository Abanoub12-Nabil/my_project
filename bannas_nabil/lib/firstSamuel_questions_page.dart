...
class FirstSamuelQuestionsPage extends StatefulWidget {
  const FirstSamuelQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _FirstSamuelQuestionsPageState createState() =>
      _FirstSamuelQuestionsPageState();
}

class _FirstSamuelQuestionsPageState extends State<FirstSamuelQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.firstSamuelQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر صموئيل الأول'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.firstSamuelQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.firstSamuelQuestions[chapter]!,
                      ),
                    );
                  },
                ),
