...
class HoseaQuestionsPage extends StatefulWidget {
  const HoseaQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _HoseaQuestionsPageState createState() => _HoseaQuestionsPageState();
}

class _HoseaQuestionsPageState extends State<HoseaQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.hoseaQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر هوشع'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.hoseaQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.hoseaQuestions[chapter]!,
                      ),
                    );
                  },
                ),
