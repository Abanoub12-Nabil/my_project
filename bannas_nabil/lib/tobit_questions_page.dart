...
class TobitQuestionsPage extends StatefulWidget {
  const TobitQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _TobitQuestionsPageState createState() => _TobitQuestionsPageState();
}

class _TobitQuestionsPageState extends State<TobitQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.tobitQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر طوبيا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.tobitQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.tobitQuestions[chapter]!,
                      ),
                    );
                  },
                ),
