...
class SecondPeterQuestionsPage extends StatefulWidget {
  const SecondPeterQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondPeterQuestionsPageState createState() =>
      _SecondPeterQuestionsPageState();
}

class _SecondPeterQuestionsPageState extends State<SecondPeterQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondPeterQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رسالة بطرس الثانية'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondPeterQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.secondPeterQuestions[chapter]!,
                      ),
                    );
                  },
                ),
