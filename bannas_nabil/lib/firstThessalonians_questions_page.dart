...
class FirstThessaloniansQuestionsPage extends StatefulWidget {
  const FirstThessaloniansQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _FirstThessaloniansQuestionsPageState createState() =>
      _FirstThessaloniansQuestionsPageState();
}

class _FirstThessaloniansQuestionsPageState
    extends State<FirstThessaloniansQuestionsPage> {
  String _searchQuery = '';
  double _selectedFontSize = 18.0; // حجم خط افتراضي

  @override
  Widget build(BuildContext context) {
    final filteredChapters = QuestionsData.firstThessaloniansQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة الأولي إلي تسالونيكي'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount = QuestionsData
              .firstThessaloniansQuestions[chapter]?.length ??
              0; // عدد الأسئلة
          return ListTile(
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        ...
                        questions: QuestionsData
                            .firstThessaloniansQuestions[chapter]!,
                      ),
                    );
                  },
                ),
