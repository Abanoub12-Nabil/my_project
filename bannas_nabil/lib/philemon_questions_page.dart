...
class PhilemonQuestionsPage extends StatefulWidget {
  const PhilemonQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _PhilemonQuestionsPageState createState() => _PhilemonQuestionsPageState();
}

class _PhilemonQuestionsPageState extends State<PhilemonQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.philemonQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة الرسالة إلي فليمون'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.philemonQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.philemonQuestions[chapter]!,
                      ),
                    );
                  },
                ),
