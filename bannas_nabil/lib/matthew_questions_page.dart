...
class MatthewQuestionsPage extends StatefulWidget {
  const MatthewQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _MatthewQuestionsPageState createState() => _MatthewQuestionsPageState();
}

class _MatthewQuestionsPageState extends State<MatthewQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.matthewQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة إنجيل متى'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.matthewQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.matthewQuestions[chapter]!,
                      ),
                    );
                  },
                ),
