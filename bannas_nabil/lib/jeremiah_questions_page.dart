...
class JeremiahQuestionsPage extends StatefulWidget {
  const JeremiahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JeremiahQuestionsPageState createState() => _JeremiahQuestionsPageState();
}

class _JeremiahQuestionsPageState extends State<JeremiahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.jeremiahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر إرميا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.jeremiahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.jeremiahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
