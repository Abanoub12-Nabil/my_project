...
class NehemiahQuestionsPage extends StatefulWidget {
  const NehemiahQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _NehemiahQuestionsPageState createState() => _NehemiahQuestionsPageState();
}

class _NehemiahQuestionsPageState extends State<NehemiahQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.nehemiahQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر نحميا'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.nehemiahQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.nehemiahQuestions[chapter]!,
                      ),
                    );
                  },
                ),
