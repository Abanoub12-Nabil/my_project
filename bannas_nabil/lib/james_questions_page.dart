...
class JamesQuestionsPage extends StatefulWidget {
  const JamesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JamesQuestionsPageState createState() => _JamesQuestionsPageState();
}

class _JamesQuestionsPageState extends State<JamesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.jamesQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رسالة يعقوب'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.jamesQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.jamesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
