...
class EcclesiastesQuestionsPage extends StatefulWidget {
  const EcclesiastesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _EcclesiastesQuestionsPageState createState() =>
      _EcclesiastesQuestionsPageState();
}

class _EcclesiastesQuestionsPageState extends State<EcclesiastesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.ecclesiastesQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر الجامعة'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.ecclesiastesQuestions[chapter]?.length ??
                  0; // خاص بعدد الاسئلة اللي قدام الاصحاحات
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.ecclesiastesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
