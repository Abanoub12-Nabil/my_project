...
class NumbersQuestionsPage extends StatefulWidget {
  const NumbersQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _NumbersQuestionsPageState createState() => _NumbersQuestionsPageState();
}

class _NumbersQuestionsPageState extends State<NumbersQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.numbersQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر العدد'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.numbersQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.numbersQuestions[chapter]!,
                      ),
                    );
                  },
                ),
