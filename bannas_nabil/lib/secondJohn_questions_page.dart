...
class SecondJohnQuestionsPage extends StatefulWidget {
  const SecondJohnQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _SecondJohnQuestionsPageState createState() =>
      _SecondJohnQuestionsPageState();
}

class _SecondJohnQuestionsPageState extends State<SecondJohnQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.secondJohnQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة رسالة يوحنا الثانية'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.secondJohnQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.secondJohnQuestions[chapter]!,
                      ),
                    );
                  },
                ),
