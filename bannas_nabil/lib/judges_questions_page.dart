...
class JudgesQuestionsPage extends StatefulWidget {
  const JudgesQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JudgesQuestionsPageState createState() => _JudgesQuestionsPageState();
}

class _JudgesQuestionsPageState extends State<JudgesQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.judgesQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر القضاة'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.judgesQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.judgesQuestions[chapter]!,
                      ),
                    );
                  },
                ),
