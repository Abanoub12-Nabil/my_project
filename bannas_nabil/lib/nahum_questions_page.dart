...
class NahumQuestionsPage extends StatefulWidget {
  const NahumQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _NahumQuestionsPageState createState() => _NahumQuestionsPageState();
}

class _NahumQuestionsPageState extends State<NahumQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.nahumQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر ناحوم'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.nahumQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions: QuestionsData.nahumQuestions[chapter]!,
                      ),
                    );
                  },
                ),
