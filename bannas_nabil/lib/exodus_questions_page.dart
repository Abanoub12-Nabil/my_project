...
class ExodusQuestionsPage extends StatefulWidget {
  const ExodusQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _ExodusQuestionsPageState createState() => _ExodusQuestionsPageState();
}

class _ExodusQuestionsPageState extends State<ExodusQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    QuestionsData.exodusQuestions.keys.where((chapter) {
      return chapter.toString().contains(_searchQuery);
    }).toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر الخروج'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.exodusQuestions[chapter]?.length ??
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.exodusQuestions[chapter]!,
                      ),
                    );
                  },
                ),
