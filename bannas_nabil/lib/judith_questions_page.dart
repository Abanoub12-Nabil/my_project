...
class JudithQuestionsPage extends StatefulWidget {
  const JudithQuestionsPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _JudithQuestionsPageState createState() => _JudithQuestionsPageState();
}

class _JudithQuestionsPageState extends State<JudithQuestionsPage> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filteredChapters = QuestionsData.judithQuestions.keys
        .where((chapter) => chapter.toString().contains(_searchQuery))
        .toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('أسئلة سفر يهوديت'),
        actions: [
          IconButton(
      ...
      body: filteredChapters.isEmpty
        ...
        children: filteredChapters.map((chapter) {
          final questionCount =
              QuestionsData.judithQuestions[chapter]?.length ??
                  0; // عدد الأسئلة
          return ListTile(
                  ...
                  pageBuilder: (context, animation, secondaryAnimation) {
                    ...
                    return SlideTransition(
                      ...
                      child: ChapterQuestionsPage(
                        chapter: chapter,
                        questions:
                        QuestionsData.judithQuestions[chapter]!,
                      ),
                    );
                  },
                ),
